...
fn main() {
    ...
    // Check for "emit-tokens" subcommand
    if args.len() >= 2 && args[1] == "emit-tokens" {
        return run_emit_tokens(&args);
    }

    // Check for "emit-ast" subcommand
    if args.len() >= 2 && args[1] == "emit-ast" {
...
fn run_emit_tokens(args: &[String]) {
    if args.len() < 3 {
        eprintln!("Usage: aven emit-tokens <file>");
        std::process::exit(1);
    }
    ...
    let tokens = aven_seed::emit_tokens_str(&source);
    println!("{}", aven_seed::canonical_token_dump(&tokens));
}

fn run_emit_ast(args: &[String]) {
    if args.len() < 3 {
        eprintln!("Usage: aven emit-ast <file>");
        std::process::exit(1);
    }
    ...
    match aven_seed::emit_ast_str(&source) {
        Ok(expr) => println!("{}", aven_seed::canonical_ast_dump(&expr)),
        Err(e) => {
            eprintln!("Parse error: {}", e);
            std::process::exit(1);
        }
